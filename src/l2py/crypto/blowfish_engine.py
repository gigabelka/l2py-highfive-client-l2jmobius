...
чтобы обеспечить совместимость с сервером.
"""
...
class BlowfishEngine:
    ...
    def __init__(self) -> None:
        ...
        self.S1 = [0] * self.SBOX_SK
        self.S2 = [0] * self.SBOX_SK
    ...
    def _process_table(self, xl: int, xr: int, table: list[int]) -> None:
        ...
        while s < size:
            ...
            xr ^= self._func(xl) ^ self.P[3]
            xl ^= self._func(xr) ^ self.P[4]
            xr ^= self._func(xl) ^ self.P[5]
            xl ^= self._func(xr) ^ self.P[6]
            xr ^= self._func(xl) ^ self.P[7]
            xl ^= self._func(xr) ^ self.P[8]
            xr ^= self._func(xl) ^ self.P[9]
            xl ^= self._func(xr) ^ self.P[10]
            xr ^= self._func(xl) ^ self.P[11]
            xl ^= self._func(xr) ^ self.P[12]
            xr ^= self._func(xl) ^ self.P[13]
            xl ^= self._func(xr) ^ self.P[14]
            xr ^= self._func(xl) ^ self.P[15]
            xl ^= self._func(xr) ^ self.P[16]
            xr ^= self.P[17]
            table[s] = xr & 0xffffffff
            table[s + 1] = xl & 0xffffffff
            xr = xl
            xl = table[s]
            s += 2
    ...
    def _set_key(self, key: bytes) -> None:
        ...
        for i in range(self.P_SZ):
            data = 0
            for _ in range(4):
                data = (data << 8) | (key[key_index] & 0xff)
                key_index += 1
    ...
    def decrypt_block(self, src: bytes, src_index: int = 0,
        ...
        xr ^= self._func(xl) ^ self.P[14]
        xl ^= self._func(xr) ^ self.P[13]
        xr ^= self._func(xl) ^ self.P[12]
        xl ^= self._func(xr) ^ self.P[11]
        xr ^= self._func(xl) ^ self.P[10]
        xl ^= self._func(xr) ^ self.P[9]
        xr ^= self._func(xl) ^ self.P[8]
        xl ^= self._func(xr) ^ self.P[7]
        xr ^= self._func(xl) ^ self.P[6]
        xl ^= self._func(xr) ^ self.P[5]
        xr ^= self._func(xl) ^ self.P[4]
        xl ^= self._func(xr) ^ self.P[3]
