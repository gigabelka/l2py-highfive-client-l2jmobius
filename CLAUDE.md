...
- [docs/RACES_CLASSES.md](docs/RACES_CLASSES.md) — playable races, full `classId` tree (103 classes), profession-change gates (L20/40/76), per-class base stats and HP/MP/CP growth curves, starting inventory per root class, XP milestones, inventory-slot caps. Generated from L2JMobius server XML + `PlayerClass.java` enum by [scripts/gen_races_classes_doc.py](scripts/gen_races_classes_doc.py); rerun to refresh.
...
- [docs/ACTIONS.md](docs/ACTIONS.md) — in-game interactive actions: targeting (`Action 0x1F`), attack (`AttackRequest 0x32`), skill (`RequestMagicSkillUse 0x39`), item (`UseItem 0x19`), drop (`RequestDropItem 0x17`), bypass (`RequestBypassToServer 0x23`), target cancel (`RequestTargetCancel 0x48`), shortcuts (`RequestShortcutReg 0x3D` / `RequestShortcutDel 0x3F`), and the `RequestActionUse 0x56` action-id catalogue (sit/stand, walk/run, pet & servitor commands, socials, airship, couple actions, summon skills 1000–1098 and 5000–5015), plus `ACTIONS_ON_TRANSFORM` allow-list and F1–F12 resolution. Generated from the L2JMobius Java source by [scripts/gen_actions_doc.py](scripts/gen_actions_doc.py); rerun to refresh.
- [docs/MOBS.md](docs/MOBS.md) — catalogue of every combat-relevant NPC (monsters, raid/grand bosses, chests, festival/rift invaders, beasts, guards/defenders/fort commanders, quest guards, decoys, friendly mobs). Per-mob: id, name, level, race, HP, pAtk/mAtk, pDef/mDef, attack range, exp/sp, aggro radius, skill count, drop/spoil counts. Full per-boss drop + spoil tables appendix for RaidBoss/GrandBoss. Merchants, teleporters, trainers, pets, summons, village masters, doormen and static towers are excluded. Generated from the L2JMobius server XML by [scripts/gen_mobs_doc.py](scripts/gen_mobs_doc.py); rerun to refresh.
